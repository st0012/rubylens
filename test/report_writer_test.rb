...
class ReportWriterTest < Minitest::Test
  ...
  def test_writes_an_offline_owner_only_report_and_protects_default_directory
    Dir.mktmpdir("rubylens-report-") do |directory|
      ...
      assert_includes(html, "const cameraDistance = 230 + workspaceRadius * .9")
      assert_includes(html, "const perspective = cameraFocal / (cameraDistance - z2) * targetZoom")
      ...
      assert_includes(html, 'const scaleAdaptiveMode = model.schema === "rubylens.art.v9" || model.schema === "rubylens.showcase.v3"')
      assert_includes(html, "const workspaceRadius = model.workspaceRadius / 1000")
      assert_includes(html, "if (qaMode)")
      assert_includes(html, "if (!showcaseMode || points.length <= SHOWCASE_POINT_LIMIT) return points")
      assert_includes(html, 'systemsTitle.textContent = "Core regions"')
      ...
      assert_includes(html, "if (showcaseMode) return [[0, renderPoints.length]]")
      assert_includes(html, "const packageMidDrawRanges = model.packages.map(() => [])")
      ...
      assert_includes(html, "window.RubyLensCoreRegions = Object.freeze")
      assert_includes(html, "function createSceneRenderer()")
      assert_includes(html, 'dataset.rubylensRenderer = "webgl2"')
      assert_includes(html, 'liveCanvas.addEventListener("webglcontextlost"')
      assert_includes(html, "(sceneCenterX + panX) * renderScale")
      assert_includes(html, "const pickingPoints = activePickingPoints()")
      assert_includes(html, "renderPoints.slice(0, midPointCount).filter(point => Number.isInteger(point.sourceIndex))")
      assert_includes(html, "renderPoints.slice(0, essentialMidPointCount).filter(point => Number.isInteger(point.sourceIndex))")
      assert_includes(html, "const activeOverviewPickPoints = () => configuredMobile() ? mobileOverviewPickPoints : overviewPickPoints")
      assert_includes(html, "return nearestScreenPoint(activeOverviewPickPoints(), x, y)")
      assert_includes(html, "systemHubAt(x, y) || nearestScreenPoint(activeOverviewPickPoints(), x, y)")
      assert_includes(html, "if (focusedGroupIndex === null) return activeOverviewPickPoints()")
      refute_includes(html, "boundedRangeSample(namespacePoints, 0, namespacePoints.length, OVERVIEW_PICK_LIMIT)")
      refute_includes(html, "nearestScreenPoint(overviewPickPoints, x, y)")
      assert_includes(html, "FOCUSED_PICK_LIMIT = 12_000")
      assert_includes(html, "u_dependencyExpansion")
      refute_includes(html, "groupedMode")
      refute_includes(html, "model.groupAnchors")
      refute_includes(html, "model.groupRadii")
