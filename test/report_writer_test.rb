...
class ReportWriterTest < Minitest::Test
  ...
  def test_writes_an_offline_owner_only_report_and_protects_default_directory
    Dir.mktmpdir("rubylens-report-") do |directory|
      ...
      assert_includes(html, "window.RubyLensPointRenderer")
      assert_includes(html, "function updateInteractionProjection")
      assert_includes(html, "cpuProjectedPoints")
      assert_includes(html, "interactionProjectionPoints")
      assert_includes(html, 'new CustomEvent("rubylensrendererfailure"')
      assert_includes(html, 'url.searchParams.set("renderer", "canvas")')
      assert_includes(html, "uniform vec3 uBounds")
