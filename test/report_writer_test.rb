...
class ReportWriterTest < Minitest::Test
  ...
  def test_writes_an_offline_owner_only_report_and_protects_default_directory
    Dir.mktmpdir("rubylens-report-") do |directory|
      ...
      model = { "schema" => "rubylens.art.v9", "projectName" => "Demo", "totals" => { "namespaces" => 2 }, "referenceRoutes" => [] }
      ...
      assert_includes(html, "expandedPackageIndex = packageIndex;\n      updateQaDrawCounts()")
      ...
      assert_includes(html, "if (routeMapOpen() || cameraFlight || selectionLocked")
      assert_includes(html, "const TOP_DOWN_PITCH = Math.PI / 2")
      assert_includes(html, "function atlasFaceOnCameraTargetForPoint")
      assert_includes(html, 'model.explorerLayout === "atlas") return atlasFaceOnCameraTargetForPoint(point, targetZoom)')
      ...
      assert_includes(html, "const detailedPoint = point.systemHub || point.hub || (!routeMapOpen()")
      ...
      assert_includes(html, "const groupedMode = Array.isArray(model.groups) && Array.isArray(model.groupRanges)")
      ...
      assert_includes(html, "const packageMidDrawRanges = groupedMode ? model.packages.map(() => []) : []")
      assert_includes(html, "for (const [first, length] of packageMidDrawRanges[expandedPackageIndex] || [])")
      assert_includes(html, "function nearestNamespaceInRange(first, length, x, y)")
      assert_includes(html, "function activateSystemRange(groupIndex")
      assert_includes(html, "if (Number.isInteger(point.groupIndex)) activateSystemRange(point.groupIndex)")
      ...
      assert_includes(html, 'id="core-routes" aria-label="Build Core route map" aria-pressed="false"')
      assert_includes(html, 'id="route-summary" aria-live="polite" tabindex="-1"')
      assert_includes(html, "function decodeReferenceRoutes")
      assert_includes(html, "decodeReferenceRoutes(model.referenceRoutes || [])")
      assert_includes(html, "delete model.referenceRoutes")
      assert_includes(html, "function buildSelectedRouteCache")
      assert_includes(html, "function insertBoundedRouteEntry")
      assert_includes(html, "if (changed) refreshSelectedRouteCache()")
      assert_includes(html, "const entries = selectedRouteEntries()")
      assert_includes(html, 'return left.direction === "outgoing" ? -1 : 1')
      refute_includes(html, "const referenceRoutes = []")
      refute_includes(html, "referenceRoutes.push")
      assert_includes(html, "function startCoreRouteMap")
      assert_includes(html, "function buildRouteMapChunk")
      assert_includes(html, "const drawRanges = routeMapOpen() ? [[0, renderPoints.length]] : visibleDrawRanges()")
      assert_includes(html, "Building Core route map")
      assert_includes(html, "Core route map")
      assert_includes(html, "route destinations are also available as buttons in the explorer")
      refute_includes(html, "All routes")
