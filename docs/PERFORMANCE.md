...
RubyLens first removes model-ineligible synthetic declarations: anonymous namespaces, Rubydex Todo placeholders, and nested singleton classes. Package declaration totals, eligible dependency-declaration totals, Ruby construct totals, signal maxima, and sampling are exact over the remaining model-eligible declaration stream. Sampled dependency declaration rows are capped globally at 18,000. This is a plotted-detail limit, not a dependency count: the historical art-model field `renderedDependencyStars` contains the number of plotted declaration rows. Every nonempty package retains a representative row when the number of nonempty packages fits inside that budget. If nonempty packages exceed the budget, seeded reservoir selection chooses which packages retain representatives, so later manifest packages remain eligible without exceeding the global cap. Eligible declarations without a sampled row still contribute to exact totals and signal maxima.
...
Because `packages[].declarations` is now bounded instead of complete, the internal handoff is `rubylens.snapshot.v6`. The model builder continues to accept the old v4 shape, where declaration arrays are complete and aggregate count/maxima fields are absent. The persisted public artifact is `rubylens.art.v8`: exact package populations, dependency-declaration totals, signal domains, and deterministic rendering retain their existing meanings.

The fixed default keeps the snapshot, standalone HTML, and plotted dependency-row population bounded independently of raw dependency volume. Raising it adaptively from total scene size would require retaining a larger declaration sample during the current single-pass index flow, or making a second declaration pass, before final namespace cardinality is known. Hierarchical or weighted dependency marks would instead change the art-model meaning and the renderer's layout, picking, and expansion behavior. An unrestricted local override would expose generation memory, HTML size, Canvas fallback, and mobile costs that have not been calibrated. Those designs remain evidence-gated; changing the default requires end-to-end public-safe proof rather than only an aggregation benchmark.
