...
The internal handoff remains `rubylens.snapshot.v6`. `packages[].declarations` contains every eligible row for every caller. `DependencyAggregation` and `RubydexAdapter` do not expose a row-limit or sampling configuration. The persisted public artifact remains `rubylens.art.v9`: exact package populations, dependency totals, signal domains, deterministic rendering, and the integer morphology block retain their existing meanings.
