...
Explorer and Showcase deliberately use different row budgets. Explorer retains every eligible dependency declaration in its snapshot and standalone HTML so WebGL2 can plot the complete dependency scene. If WebGL2 is unavailable, rejected by the point-size capability check, fails to initialize, or loses its context, the runtime plots no scene and opens the standard warning. Exact totals and gem aggregates remain available in the report, but interaction requires WebGL2 rather than silently changing the visualized population.

The internal handoff remains `rubylens.snapshot.v6`. `packages[].declarations` contains every eligible row for Explorer and a bounded deterministic sample for Showcase and direct `GenerationPipeline` use. The model builder continues to accept the old v4 shape, where declaration arrays are complete and aggregate count/maxima fields are absent. The persisted public artifact remains `rubylens.art.v9`: exact package populations, dependency totals, signal domains, deterministic rendering, and the integer morphology block retain their existing meanings.

Morphology classification reads only the snapshot's existing namespace rows, namespace names, exact dependency declaration totals, and project name. It performs no indexing, file walking, or subprocess discovery. The renderer still decodes morphology once, precomputes at most five irregular clump centres, then evaluates each Core/Test position in O(1); the different Explorer and Showcase artifact budgets do not change those calculations.

Showcase and direct `GenerationPipeline` use the bounded aggregation default before art-model construction. Their order-independent priority sample is capped globally at 18,000 rows, retains a representative from every nonempty package when possible, and keeps eligible declarations without a retained row in exact totals, Ruby construct counts, and signal maxima. This prevents dependency volume from becoming an equally large Showcase snapshot before its separate 50,000-point scene projection.

These artifact-specific budgets do not change the `rubylens.snapshot.v6` or `rubylens.art.v9` schemas. `totals.dependencyStars` is the exact eligible declaration total. `totals.renderedDependencyStars` is the number of declaration rows embedded in that artifact, not a dependency or gem count. Explorer runtime state exposes that full embedded count as plotted under WebGL2 and zero when interactive rendering is unavailable.
