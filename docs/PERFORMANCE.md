...
The internal handoff is `rubylens.snapshot.v7`. `packages[].declarations` contains every eligible row for every caller. `DependencyAggregation` and `RubydexAdapter` do not expose a row-limit or sampling configuration. The persisted Explorer art model is `rubylens.art.v11`, and both Showcase projections use `rubylens.showcase.v5`. All carry signal domains, the project morphology as one ten-integer row, complete package and dependency populations, and one aligned integer `packageMorphologies` row per package; the Showcase schema is the privacy-filtered projection used by its Minimal and Details surfaces.
