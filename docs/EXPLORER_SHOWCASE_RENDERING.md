...
Both surfaces also share deterministic package-cloud rotation and a transient 2D travel overlay derived from bounded constant-reference candidates. Neither adds scene points or persistent edges.
...
| GPU data | One shared static point buffer plus a derived one-texel-per-package anchor/spin texture; package and system indices also drive focus/expansion | The same shared point buffer and derived package texture; interaction-only indices remain inert |
...
Both WebGL2 vertex shaders apply the same transform from a one-texel-per-package float texture. Dependency haze retains its package and system indices, so the resampled micro-stars rotate with their parent cloud instead of leaving its texture behind. This keeps the point buffer static and adds no per-star CPU work or buffer upload. CPU projection applies the identical transform only for the bounded dependency endpoints used by travel admission and active-flight reprojection. Explorer accumulates its cloud clock only while drift is active; Showcase and Clip use presentation elapsed time, with reduced motion fixed at phase zero. Whole turns close the 60-second Clip seam exactly.
...
- for each scheduled launch, admission projects both endpoints at its expected mid-flight camera and cloud-spin phase, chooses one stable arc side, and checks the candidate curve's complete bounds and minimum 48-pixel length once; it does not sample other camera or cloud-spin states during admission;
...
- each admitted 2.2-second flight remains one continuous immutable episode while its two endpoints are reprojected at the current camera and cloud-spin phase and its same-side quadratic is rebuilt for each drawn frame; manual Explorer camera changes, viewport resizes, or category visibility changes reset its travel clock and timetable, so the next episode observes the normal launch delay instead of jumping or re-entering a route;
...
- a long, round-capped same-hue wake feathers in and thickens beneath one filled elongated drop, then both fade out without a separate white-hot tip, moving orb, or persistent edge;
...
The continuous timetable is deterministic for a generated artifact. Each frame binary-searches the bounded minute and scans backward only across the fixed 2.2-second lifetime, then reprojects at most four endpoints at the current camera and cloud-spin phase and draws at most two fixed-segment trails without scanning the candidate list. The hard-capped overlay therefore remains O(1) per frame. Minimal Showcase carries the same anonymous candidate rows as Details, which discloses a sparse sample of relationship topology without endpoint names.
