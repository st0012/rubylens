    ...
    (() => {
      const VERTEX_SHADER = `#version 300 es
        ...
        void main() {
          ...
          gl_Position=vec4(clip, 0.0, depth>35.0?1.0:0.0);
          float hub=aStyle.w;
          float size=clamp(aStyle.x*(0.62+aStyle.y*0.46)*perspective,0.35,hub>0.5?5.2:3.2);
          gl_PointSize=selected>0.5?max(12.0,size*8.0):(size<0.85?1.0:size*2.4);
      ...
      const FRAGMENT_SHADER = `#version 300 es
        ...
        void main() {
          ...
          if(vSelected>0.5 && distance>0.72 && distance<0.88){colour=vec4(1.0);return;}
          float core=smoothstep(1.0,0.0,distance);
          float glow=smoothstep(1.0,0.15,distance)*0.12;
      ...
      class WebGLPointRenderer {
        constructor(canvas) {
          ...
          canvas.addEventListener("webglcontextrestored",()=>{this.lost=false;this.initializeResources();if(this.points)this.sync(this.points);});
        }
        initializeResources() {
        ...
        sync(points) {
          ...
          this.points=points; this.count=points.length; this.needsSync=false; this.uploadMilliseconds=performance.now()-started;
        ...
        draw(frame) {
          ...
          const gl=this.gl; gl.clearColor(3/255,4/255,10/255,1); gl.clear(gl.COLOR_BUFFER_BIT);
          ...
          const selectedIndex=frame.selectedPoint?this.points.indexOf(frame.selectedPoint):-1;
