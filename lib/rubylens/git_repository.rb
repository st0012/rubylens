...
module RubyLens
  class GitRepository
    ...
    def selected_files
      enumerate_files("--cached", "--others", "--exclude-standard", error: "failed to enumerate tracked and unignored files")
    end

    def tracked_files
      enumerate_files("--cached", error: "failed to enumerate tracked files")
    end
    ...
    def enumerate_files(*arguments, error:)
      output, status = capture("ls-files", "-z", *arguments)
      raise ExtractionError, error unless status.success?
